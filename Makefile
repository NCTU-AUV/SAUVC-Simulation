...
launch: compose_up
	$(ROS_NET_ENV) $(COMPOSE) exec $(ROS_EXEC_ENV) orca /bin/bash -lc "\
		...
		WEBSOCKET_PID=\$$!; \
		ros2 launch bringup orca_ros_gz_bridge_launch.py namespace:=$(NAMESPACE) world:=$(WORLD) headless:=true & \
		BRIDGE_PID=\$$!; \
		trap 'kill \$$WEBSOCKET_PID \$$BRIDGE_PID 2>/dev/null; wait 2>/dev/null' INT TERM EXIT; \
		wait -n \$$WEBSOCKET_PID \$$BRIDGE_PID"
